...
<template>
  <!-- Vista normal — fila completa abre el perfil -->
  <div
    class="list-container list-container--clickable"
    v-if="!editionActive && !deleteActive"
    @click="emit('open-profile', char)"
  >
    ...
    <span>
      <div class="buttons-container">
        <button class="icon-button" v-if="isLeader || isOfficer" @click.stop="handleEdit" title="Editar"><i class="fas fa-pen"></i></button>
        <button class="icon-button icon-button--danger" v-if="role !== 'leader' && (isLeader || role === 'member')" @click.stop="handleDelete" title="Eliminar"><i class="fas fa-trash"></i></button>
      </div>
    </span>
  </div>
