...
const chartData = computed(() => {
  const s = data.value?.shadowWar ?? { victory: 0, defeat: 0, draw: 0 };
  return {
...
<template>
  <div class="statistics-detail">
    ...
    <h3 class="detail-title"><i class="fas fa-khanda"></i> Guerra Sombría</h3>
    ...
    <template v-else>
      ...
      <EmptyState
        v-if="!data?.shadowWar?.matches?.length"
        icon="fas fa-scroll"
        message="No hay Shadow Wars con resultado en este rango."
        :compact="true"
      />

      <TableComponent v-else :navItems="navItems">
        <div v-for="match in data.shadowWar.matches" :key="match._id" class="detail-row">
